...
function guardarDatos(datos) {  
  try {
    ...
    const datosInteresDenunciadoTexto = Array.isArray(datos.datosInteresDenunciado) && datos.datosInteresDenunciado.length > 0 
      ? datos.datosInteresDenunciado.join('\n') 
      : '';

    Logger.log('Tipo de datosInteresDenunciado: ' + typeof datos.datosInteresDenunciado);
    Logger.log('¿Es array?: ' + Array.isArray(datos.datosInteresDenunciado));
    Logger.log('Contenido: ' + JSON.stringify(datos.datosInteresDenunciado));

    // Definir cuentasPagoTexto aquí
    const cuentasPagoTexto = Array.isArray(datos.cuentasPago) ? 
      datos.cuentasPago.join('\n') : datos.cuentaPago || '';
    console.log('Datos originales:', datos.datosInteresDenunciado);
    console.log('Tipo de dato:', typeof datos.datosInteresDenunciado);
    console.log('Texto procesado:', datosInteresDenunciadoTexto);
    sheet.appendRow([
      datos.id,
      new Date(), 
...
function actualizarRegistroEnSheet(datos) {
  try {
    ...
    function buscarFila(idRegistro) {
      // Asumiendo que el ID está en la primera columna
      for (let i = 1; i < data.length; i++) {  // Empezamos desde 1 para saltar encabezados
        if (data[i][0] === idRegistro) {
          return i + 1;  // +1 porque en la hoja las filas empiezan en 1, no en 0
        }
      }
      return null;  // No encontrado
    }    
    
    let filaEncontrada = buscarFila(datos.id);
    
    // Si no se encuentra la fila, podemos crear una nueva
    if (!filaEncontrada) {
      // Opcional: crear una nueva fila
      Logger.log("No se encontró el registro, se creará uno nuevo");
      // Código para crear nueva fila...
      // O retornar un error:
    ...
    if (filaEncontrada > 0) {
      ...
      sheet.getRange(filaEncontrada, 13).setValue(datosInteresDenunciadoTexto);
